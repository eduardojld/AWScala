...
class S3Spec extends FlatSpec with Matchers {
  ...
  it should "handle buckest with client side encryption" in {
    implicit val s3 = S3EncryptionClient.at(Region.Tokyo)
    ...
    // create/update objects
    val originalFile = new java.io.File("src/main/scala/awscala/s3/S3.scala")
    val processedFile = new File("fileEncrypt");

    bucket.put("S3.scalaEncrypt", originalFile)

    val obj = s3.getObject(bucket, "S3.scalaEncrypt", processedFile)
    val objectContent = obj.get.getObjectContent();

    new File("fileEncrypt").delete()

    s3.deleteObject(bucket.name, "S3.scalaEncrypt")
